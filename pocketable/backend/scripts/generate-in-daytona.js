...
async function generateAppInDaytona(sandboxIdArg, prompt) {
...
SCRIPT_EOF`, projectDir);
        ...
        const checkScript = await sandbox.process.executeCommand("ls -la generate.js && head -5 generate.js", projectDir);
        ...
        const genResult = await sandbox.process.executeCommand("node generate.js", projectDir, {
        ...
        const hasPackageJson = await sandbox.process.executeCommand("test -f package.json && echo yes || echo no", projectDir);
        if (hasPackageJson.result?.trim() === "yes") {
            console.log("\n9. Installing project dependencies...");
            const npmInstall = await sandbox.process.executeCommand("npm install", projectDir, undefined, 300000 // 5 minute timeout
            );
        ...
        console.log(`- To get preview URL again: npx tsx scripts/get-preview-url.ts ${sandboxId}`);
        console.log(`- To reuse this sandbox: npx tsx scripts/generate-in-daytona.ts ${sandboxId}`);
        ...
        if (sandbox) {
            ...
            try {
                const debugInfo = await sandbox.process.executeCommand("pwd && echo '---' && ls -la && echo '---' && test -f generate.js && cat generate.js | head -20 || echo 'No script'", `${await sandbox.getUserRootDir()}/expo-project`);
                console.log("\nDebug info:");
...
main();
//# sourceMappingURL=generate-in-daytona.js.map