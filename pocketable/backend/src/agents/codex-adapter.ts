...
import { REACT_NATIVE_EXPERT_PROMPT } from '../config/models';
import type { AgentSDK, ChatContext, ChatChunk } from './types';

export class CodexAdapter implements AgentSDK {
  ...
  private buildSystemPrompt(mode: 'auto' | 'plan' | 'build'): string {
    ...
    return REACT_NATIVE_EXPERT_PROMPT + modeInstructions;
  }

  // Clean up old threads to prevent memory leaks
  clearThread(projectId: string) {
