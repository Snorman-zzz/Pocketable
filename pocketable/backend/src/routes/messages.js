...
const openai_routing_1 = require("../services/openai-routing");
...
router.post('/:projectId/message', async (req, res) => {
    ...
    try {
        ...
        const userMsgResult = await database_1.databaseService.query(`INSERT INTO chat_messages (project_id, role, content)
       VALUES ($1, 'user', $2)
       RETURNING id, created_at`, [projectId, message]);
        ...
        const [historyResult, filesResult] = await Promise.all([
            ...
            database_1.databaseService.query(`SELECT role, content
         FROM chat_messages
         WHERE project_id = $1
         ORDER BY created_at DESC
         LIMIT 50`, [projectId]),
            // All project files
            database_1.databaseService.query(`SELECT file_path, content
         FROM project_files
         WHERE project_id = $1`, [projectId]),
        ]);
        const conversationHistory = historyResult.rows.reverse();
        const projectFiles = Object.fromEntries(filesResult.rows.map((f) => [f.file_path, f.content]));
        ...
        const routing = await (0, openai_routing_1.routeMessage)(projectId, message, conversationHistory, projectFiles);
       ...
       WHERE id = $3`, [routing.intent, routing.confidence, userMessageId]);
        ...
        if (routing.intent === 'BUILD') {
            ...
            const assistantMsgResult = await database_1.databaseService.query(`INSERT INTO chat_messages
         (project_id, role, content, model, routing_intent, routing_confidence, router_model)
         VALUES ($1, 'assistant', $2, 'gpt-5', 'GENERAL', $3, 'gpt-5-nano')
         RETURNING id, content, created_at`, [projectId, routing.response, routing.confidence]);
 ...
 */
router.post('/:projectId/message/stream', async (req, res) => {
    const startTime = Date.now();
    try {
        const { projectId } = req.params;
        const { message, model } = req.body;
       ...
       RETURNING id, created_at`, [projectId, message]);
        ...
        const classification = await (0, openai_routing_1.classifyIntent)(message);
       ...
       SET routing_intent = $1,
           routing_confidence = $2,
           router_model = 'gpt-5-nano'
       WHERE id = $3`, [classification.intent, classification.confidence, userMessageId]);
        // === STEP 5: Stream GENERAL response ===
        let fullResponse = '';
        let wasCompacted = false;
        try {
            for await (const event of (0, openai_routing_1.generatePocketableResponseStream)(projectId, message, conversationHistory, projectFiles)) {
                if (event.chunk) {
         ...
         (project_id, role, content, model, routing_intent, routing_confidence, router_model)
         VALUES ($1, 'assistant', $2, 'gpt-5', 'GENERAL', $3, 'gpt-5-nano')
         RETURNING id, created_at`, [projectId, fullResponse, classification.confidence]);
            const totalLatency = Date.now() - startTime;
