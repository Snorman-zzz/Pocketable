...
router.get('/projects/:projectId/snapshots', async (req, res) => {
    try {
        ...
        const snapshots = await snapshot_service_1.snapshotService.getProjectSnapshots(projectId);
        res.json({
            success: true,
            snapshots,
...
router.get('/:snapshotId', async (req, res) => {
    try {
        ...
        const snapshot = await snapshot_service_1.snapshotService.getSnapshot(snapshotId);
        if (!snapshot) {
 ...
 */
router.get('/:snapshotId/files', async (req, res) => {
    try {
        const { snapshotId } = req.params;
        const files = await snapshot_service_1.snapshotService.getSnapshotFiles(snapshotId);
        if (!files) {
            return res.status(404).json({
                success: false,
                error: 'Snapshot not found',
 ...
 */
router.post('/projects/:projectId/restore/:snapshotId', async (req, res) => {
    try {
        ...
        const result = await databaseService.query(`INSERT INTO chat_messages (project_id, role, content, is_edit_card, snapshot_id)
       ...
       RETURNING *`, [
        ...
        await snapshot_service_1.snapshotService.updateProjectCurrentSnapshot(projectId, snapshotId);
...
router.delete('/:snapshotId', async (req, res) => {
    try {
        const { snapshotId } = req.params;
        const { databaseService } = await import('../services/database');
        const result = await databaseService.query(`DELETE FROM code_snapshots WHERE id = $1 RETURNING id`, [snapshotId]);
