...
class SnapshotService {
  ...
  async createSnapshot(
    ...
    const result: QueryResult<CodeSnapshot> = await databaseService.query(
      `INSERT INTO code_snapshots (project_id, message_id, files, sandbox_id, preview_url, build_status)
       VALUES ($1, $2, $3, $4, $5, 'success')
       RETURNING *`,
      [projectId, messageId, JSON.stringify(files), sandboxId || null, previewUrl || null]
    );
  ...
  async getSnapshot(snapshotId: string): Promise<CodeSnapshot | null> {
    ...
    const result: QueryResult<CodeSnapshot> = await databaseService.query(
      `SELECT * FROM code_snapshots WHERE id = $1`,
      [snapshotId]
    );
   ...
   */
  async getProjectSnapshots(projectId: string): Promise<SnapshotWithMessage[]> {
    if (!databaseService.isAvailable()) {
      throw new Error('Database not available');
    }

    const result: QueryResult<SnapshotWithMessage> = await databaseService.query(
      `SELECT
        cs.id,
        cs.project_id,
        cs.message_id,
   ...
   * @param snapshotId - Snapshot UUID
   */
  async markMessageAsEditCard(messageId: string, snapshotId: string): Promise<void> {
    if (!databaseService.isAvailable()) {
      throw new Error('Database not available');
    }
   ...
   * @param snapshotId - Snapshot UUID
   */
  async updateProjectCurrentSnapshot(projectId: string, snapshotId: string): Promise<void> {
    if (!databaseService.isAvailable()) {
      throw new Error('Database not available');
    }
   ...
   */
  async updateProjectSandbox(projectId: string, sandboxId: string | null): Promise<void> {
    if (!databaseService.isAvailable()) {
      throw new Error('Database not available');
    }
   ...
   */
  async getLastAssistantMessage(projectId: string): Promise<{ id: string; content: string } | null> {
    if (!databaseService.isAvailable()) {
      throw new Error('Database not available');
    }

    const result: QueryResult<{ id: string; content: string }> = await databaseService.query(
   ...
   */
  async getSnapshotFiles(snapshotId: string): Promise<Record<string, string> | null> {
   ...
   */
  async cleanupOldSnapshots(projectId: string, keepCount: number = 50): Promise<number> {
    if (!databaseService.isAvailable()) {
      throw new Error('Database not available');
    }
